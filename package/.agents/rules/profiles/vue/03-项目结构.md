...
上表与目录树中的 `.ts`、`.tsx` 为 TypeScript 示例。**纯 JavaScript 项目**将对应文件改为 `.js` / `.jsx`（如 `main.js`、`router/modules/*.js`）。入口、路由、API、Mock、`types` 等路径须与 `src/` 实际一致；使用「初始化项目规范」技能时以扫描结果为准。
