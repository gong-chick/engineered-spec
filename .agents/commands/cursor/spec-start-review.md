---
name: /spec-start-review
id: spec-start-review
category: Workflow
description: Cursor（开发工具）兼容入口：默认启用人工审核，并支持命令尾部 flags（标志位）
---

这是 Cursor（开发工具） 的人工审核协议入口，不是直接开发命令。
...
9. 若 `turn.enforcement.current_command_finalizes_run = true`，说明当前命令已完成归档与运行收尾；不要再写 `expert-execution（专家执行结果）` JSON，不要再执行 `protocol-advance（协议推进命令）`
